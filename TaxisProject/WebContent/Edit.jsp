...
<head>
	   ...
	   <script src="resources/js/Future.js"></script>
		...
		<script src="resources/js/AjaxEditPage.js"></script>
 ...
 <a href="?action=edit&sessionLocale=en"><img src="resources/images/UK.png" width="30" 
   height="30"></a>
	<a href="?action=edit&sessionLocale=ru"><img src="resources/images/RU.png" width="30" 
   height="30"></a></div>
		...
		<br>
		 <form action="/TaxisProject/home?action=editResult"  method="post">
 ...
 <option selected value=""><fmt:message key="text.chooseCar" /></option>
   <c:forEach items="${taxis}" var="entry">
    <option data-car ="${entry.value.car}" 
    data-carModel="${entry.value.carModel}" 
    data-cost="${entry.value.cost}" 
    data-fuelConsumption="${entry.value.fuelConsumption}" 
    data-acceleration="${entry.value.acceleration}"
     data-maxSpeed="${entry.value.maxSpeed}"
      data-priceForOneMinute="${entry.value.priceForOneMinute}"
     data-id="${entry.value.id}"
    value="${entry.value.id}" >${entry.value.classCar},${entry.value.car},${entry.value.carModel}</option>
   </c:forEach> 
  
 </select> 
</span>
<h3><fmt:message key="text.enterValue" /></h3> 

<input  type="hidden"   name="id"  >
<input  type="text"  placeholder="${Car}" name="car" value="" required/>
<input type="text" placeholder="${carModel}" name="carModel" value="" required/><br>
<input type="number" step=".01" min="0.1" placeholder="${cost}" name="cost" value="" required/>
<input type="number" step="1" min="1" placeholder="${fuelConsumption}" name="fuelConsumption" value="" required/><br>
<input type="number" step=".01" min="0.1" placeholder="${acceleration}" name="acceleration" value="" required/>
<input type="number" step=".01" min="0.1" placeholder="${maximumSpeed}" name="maxSpeed" value="" required/><br>
<input type="number" step=".01" min="0.1" placeholder="${priceForOneMinute}" name="priceForOneMinute" value="" required/>
  <br>
  
     <button class="btn_login" ><fmt:message key="text.update" /></button> 
 </form>
	        <br>
			</div>
